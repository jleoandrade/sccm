# 1. Define the machine target list
# You can manually populate this array or load it dynamically (e.g., Get-Content -Path "C:\temp\computers.txt")
$ComputerList = @(
...
foreach ($Computer in $ComputerList) {
    ...
    try {
        $Result = Invoke-Command -ComputerName $Computer -ScriptBlock {
            ...
            # Sequential invocation of targeted SCCM Client Evaluation Schedules (0113, 0021, and 0022)
            try {
                Write-Host "[$env:COMPUTERNAME] Executing SCCM Triggers: 0113, 0021, 0022..."
                $Triggers = @("{00000000-0000-0000-0000-000000000113}", "{00000000-0000-0000-0000-000000000021}", "{00000000-0000-0000-0000-000000000022}")
                
                foreach ($Trigger in $Triggers) {
                    Invoke-WmiMethod -Namespace "root\ccm" -Class "SMS_Client" -Name "TriggerSchedule" -ArgumentList $Trigger -ErrorAction Stop | Out-Null
                    Start-Sleep -Seconds 2 # Operational buffer delay between schedule system calls
                }
                $Output.SCCMTriggersExecuted = $true
            }
            catch {
                $Output.SCCMTriggersExecuted = $false
                $Output.Error += " | Triggers Error: " + $_.Exception.Message
            }
        ...
        Write-Host "[LOCAL] CRITICAL ERROR executing Invoke-Command on $Computer : $_" -ForegroundColor Red
    }
}
