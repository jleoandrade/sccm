<#
    Script: Local KB Installer + SCCM Sync (Windows 11 23H2 / 24H2 / 25H2)
#>

Write-Host "=== Local Windows 11 KB Installer (23H2 / 24H2 / 25H2) ===" -ForegroundColor Cyan

Write-Host "Collecting Windows information..." -ForegroundColor Cyan

$cv = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion"
$displayVersion = $cv.DisplayVersion
$releaseId      = $cv.ReleaseId
$build          = $cv.CurrentBuild
$ubr            = $cv.UBR

Write-Host "Detected version:" -ForegroundColor Yellow
Write-Host "  DisplayVersion : $displayVersion"
Write-Host "  ReleaseId      : $releaseId"
Write-Host "  Build          : $build.$ubr"

# Map version -> KBs
$kbList = @()
$localFolder = "C:\temp"
if (-not (Test-Path $localFolder)) {
    New-Item -Path $localFolder -ItemType Directory | Out-Null
}

switch ($displayVersion) {
    "23H2" {
        $kbList = @(
            [PSCustomObject]@{
                Id  = "KB5078883"
                Url = "https://catalog.sf.dl.delivery.mp.microsoft.com/filestreamingservice/files/cc3b3a63-4780-408f-a0a9-ae941d7f0c95/public/windows11.0-kb5078883-x64_ad6670dbea0da64763fd15b25ac1d6dcd966fdca.msu"
            }
        )
    }
    "24H2" {
        $kbList = @(
            [PSCustomObject]@{
                Id  = "KB5079473"
                Url = "https://catalog.sf.dl.delivery.mp.microsoft.com/filestreamingservice/files/db81d709-c552-47ed-a3b9-ba77dfff9972/public/windows11.0-kb5079473-x64_651d0cdb7665e03bb80b4c198784b255081032d2.msu"
            },
            [PSCustomObject]@{
                Id  = "KB5043080"
                Url = "https://catalog.sf.dl.delivery.mp.microsoft.com/filestreamingservice/files/d8b7f92b-bd35-4b4c-96e5-46ce984b31e0/public/windows11.0-kb5043080-x64_953449672073f8fb99badb4cc6d5d7849b9c83e8.msu"
            }
        )
    }
    "25H2" {
        $kbList = @(
            [PSCustomObject]@{
                Id  = "KB5079473"
                Url = "https://catalog.sf.dl.delivery.mp.microsoft.com/filestreamingservice/files/db81d709-c552-47ed-a3b9-ba77dfff9972/public/windows11.0-kb5079473-x64_651d0cdb7665e03bb80b4c198784b255081032d2.msu"
            },
            [PSCustomObject]@{
                Id  = "KB5043080"
                Url = "https://catalog.sf.dl.delivery.mp.microsoft.com/filestreamingservice/files/d8b7f92b-bd35-4b4c-96e5-46ce984b31e0/public/windows11.0-kb5043080-x64_953449672073f8fb99badb4cc6d5d7849b9c83e8.msu"
            }
        )
    }
    Default {
        Write-Host "Unsupported Windows version. Aborting." -ForegroundColor Red
        return
    }
}

# Function to check if KB is installed
function Test-KBInstalled {
    param([string]$KbId)

    $foundHotfix = Get-HotFix -Id $KbId -ErrorAction SilentlyContinue
    if ($foundHotfix) { return $true }

    $dism = dism /online /get-packages | Select-String $KbId -ErrorAction SilentlyContinue
    if ($dism) { return $true }

    return $false
}

# Function to install KB
function Install-KB {
    param(
        [string]$KbId,
        [string]$Url
    )

    $msuPath = Join-Path $localFolder "$KbId.msu"

    if (Test-KBInstalled -KbId $KbId) {
        Write-Host "$KbId already installed." -ForegroundColor Green
        return
    }

    Write-Host "Downloading $KbId..." -ForegroundColor Cyan
    Start-BitsTransfer -Source $Url -Destination $msuPath

    Write-Host "Installing $KbId..." -ForegroundColor Cyan
    Start-Process "wusa.exe" -ArgumentList "`"$msuPath`" /quiet /norestart" -Wait

    Start-Sleep -Seconds 20

    if (Test-KBInstalled -KbId $KbId) {
        Write-Host "$KbId installed successfully." -ForegroundColor Green
    }
    else {
        Write-Host "KB installation failed: $KbId" -ForegroundColor Red
    }
}

Write-Host "Installing KBs..." -ForegroundColor Cyan

foreach ($kb in $kbList) {
    Install-KB -KbId $kb.Id -Url $kb.Url
}

Write-Host "Triggering SCCM sync..." -ForegroundColor Cyan

Invoke-WmiMethod -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList "{00000000-0000-0000-0000-000000000003}"
Invoke-WmiMethod -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList "{00000000-0000-0000-0000-000a000000113}"
Invoke-WmiMethod -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList "{00000000-0000-0000-0000-000000000121}"

Write-Host "Done." -ForegroundColor Green
