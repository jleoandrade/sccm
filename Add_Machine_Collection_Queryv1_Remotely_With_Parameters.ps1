...
[CmdletBinding()]
param (
    [Parameter(Mandatory = $true, HelpMessage = "Insira o FQDN ou IP do Servidor SCCM.")]
    [string]$SccmServer
)

Write-Host "=== SCCM Collection Manager (Remote Mode) ===" -ForegroundColor Cyan
Write-Host "Alvo: $SccmServer" -ForegroundColor Yellow

# ============================================================
#  CONNECT TO SCCM SERVER AND LOAD MODULE REMOTELY
# ============================================================

# Detect provider and site code remotely using CIM (Modern replacement for WMI)
try {
    $providerInfo = Get-CimInstance -ComputerName $SccmServer -Namespace "root\sms" -ClassName SMS_ProviderLocation -ErrorAction Stop | Select-Object -First 1
    ...
    # String fixed using the format operator (-f) to strictly isolate variables from colons
    $errorMessage = "Erro ao conectar ao WMI/CIM do servidor {0}: {1}" -f $SccmServer, $_.Exception.Message
    Write-Host $errorMessage -ForegroundColor Red
    return
}

# Load SCCM module from server (Caminho fixo padronizado)
...
if (-not (Test-Path $CMModulePath)) {
    Write-Host "Modulo SCCM nao encontrado no caminho padrao: $CMModulePath" -ForegroundColor Red
    $CMModulePath = Read-Host "Por favor, digite o caminho correto do arquivo ConfigurationManager.psd1"
}
...
try {
    ...
    $modErrorMessage = "Erro ao carregar o modulo do SCCM: {0}" -f $_.Exception.Message
    Write-Host $modErrorMessage -ForegroundColor Red
    return
}
...
$collections = Get-CimInstance -ComputerName $SccmServer -Namespace "root\sms\site_$SiteCode" -ClassName SMS_Collection |
...
# Garante que loops funcionem corretamente se apenas 1 coleção for retornada
$collectionList = @($collections)

for ($i = 0; $i -lt $collectionList.Count; $i++) {
    Write-Host "[$i] $($collectionList[$i].Name)"
}

$index = Read-Host "`nEnter the number of the collection you want to use"

if ($index -notmatch '^\d+$' -or $index -ge $collectionList.Count) {
